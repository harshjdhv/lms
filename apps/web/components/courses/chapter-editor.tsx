                    ...
                    <div className="rounded-lg border bg-muted/20 p-4">
                      ...
                      {initialData.transcriptJson && (
                        <div className="text-xs font-mono bg-background p-2 rounded border h-24 overflow-y-auto text-muted-foreground">
                          {(initialData.transcriptJson as any[])
                            .slice(0, 5)
                            .map((s: any, i) => (
                              <div key={i}>
                                [{Math.floor(s.start)}s] {s.text}
                              </div>
                            ))}
                          {(initialData.transcriptJson as any[]).length > 5 && (
                            <div className="mt-1 text-center italic">
                              ...and{" "}
                              {(initialData.transcriptJson as any[]).length - 5}{" "}
                              more
                            </div>
                    ...
                    <div className="rounded-lg border bg-muted/20 p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {initialData.reflectionPoints &&
                            initialData.reflectionPoints.length > 0
                        ...
                        <Button
                          ...
                          onClick={() => {
                            toast.promise(
                              ...
                              {
                                loading:
                                  "Analyzing transcript and generating points...",
                                success: (msg) => msg,
                                error: (err) => `Error: ${err.message}`,
                              },
                            );
                      ...
                      initialData.reflectionPoints.length > 0 ? (
                        <div className="space-y-1">
                          {initialData.reflectionPoints.map((p) => (
                            <div
                              key={p.id}
                              className="text-sm flex items-center justify-between p-2 bg-background rounded border"
                            >
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                        <div className="space-y-0.5">
                          ...
                          <FormDescription>
                            Reviewing this chapter? Unpublish it to hide it from
                            students.
                          </FormDescription>
                        </div>
                        <FormControl>
